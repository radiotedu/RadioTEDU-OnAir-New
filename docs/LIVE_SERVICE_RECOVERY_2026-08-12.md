...
3. Superseded by the current 32-mount contract: the broadcast PC keeps all 24 music quality branches enabled using Opus 64/96/192 and Ogg/FLAC. The external origin must provision and accept those mounts; rejected branches retry independently without stopping playout. `/en` and `/fr` remain externally owned because no AI quality mounts are provisioned here.
