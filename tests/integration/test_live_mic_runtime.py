...
class _FakeLiveMicRegistry:
    def __init__(self, mic_pcm: bytes):
        self._listeners = []
        self._mic_pcm = bytes(mic_pcm)
        self._state = {
            "station_id": 1,
    ...
    def read_pcm(self, station_id: int, num_bytes: int) -> bytes:
        ...
        chunk = self._mic_pcm[:requested]
        if len(chunk) < requested:
            chunk += b"\x00" * (requested - len(chunk))
        return chunk

    def activate(self, station_id: int, user: dict):
        self._state.update(
            {
        ...
        for listener in list(self._listeners):
            listener("start", int(station_id), snapshot)


def _make_pcm_frame(left: int, right: int) -> bytes:
    return int(left).to_bytes(2, byteorder="little", signed=True) + int(right).to_bytes(
...
def test_live_mic_start_promotes_running_station_to_live_mix(tmp_path, monkeypatch):
    ...
    music_frames = _make_pcm_frame(1000, 1000) * 2
    music_producer = _FakeProcess(stdout_chunks=[music_frames])
    steady_state = _FakeProcess(stdout_chunks=[b"ignored"])
    clock = {"value": 100.0}
    mic_registry = _FakeLiveMicRegistry(
        mic_pcm=int(500).to_bytes(2, byteorder="little", signed=True) * 2
    )
    ...
    registry = StationRuntimeRegistry(
        ...
        live_mic_registry=mic_registry,
    )
    ...
    assert "12.000" in " ".join(producer_cmd)
